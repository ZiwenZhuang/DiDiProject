 ...
 */
float KNN_removalValue(const float * imgIn, const unsigned int xIn, const unsigned int yIn, float * kernel, const unsigned int width,  const unsigned int height, const unsigned int kernelSize, const unsigned int k, const float distance_thr,const float cos_w,const float cos_h){
  ...
  // Law of cosines ....
  const float width_per_pixel = sqrt( 2*depth_squared - (cos_w*2*depth_squared) );
  const float height_per_pixel = sqrt( 2*depth_squared - (cos_h*2*depth_squared) );

  // loop through the kernel
  //#pragma omp parallel shared(kernel)
  int va=0,num=0;
  float a;
  {
    //#pragma omp for
    for(int x = 0; x < kernelSize; ++x){
      for(int y = 0; y < kernelSize; ++y){
        ...
        a = fabs(imgIn[ xIn - halfSize + x  + (yIn - halfSize + y) * width] - center_depth)
        + width_per_pixel*abs(x - halfSize) + height_per_pixel*abs(y - halfSize);
        if(a!=0.f){kernel[va]=a;va++;}
        if(a<distance_thr){num++;}
      }
    }
  }
  ...
  if(num<k){
    nth_element(kernel,kernel+k,kernel+va);

    // get the k nearest vaild neighbor 
    int start = 0;
    //#pragma omp parallel shared(start,k_sum)
    for(int i = 0; i < va && i < start + k; i++){
      if(kernel[i] == 0.f){
        start = i + 1;
      }else{
        k_sum += kernel[i];
...
void KNN_removal(const float * imgIn, int * imgOut, const unsigned int &width, const unsigned int &height, const unsigned int &kernelSize, const unsigned int k, const float distance_thr){
  ...
  const float cos_w = cos(M_PI * 2. / width );
  const float cos_h = cos(M_PI * 2. / height );
      ...
      for(int y = 0; y < height; ++y){
        ...
        if(x >= halfSize && y >= halfSize && x < width-halfSize && y < height-halfSize){
          ...
          if(imgIn[x + y*width] != 0.f){
            ...
            imgOut[x + y*width] = (int )KNN_removalValue(imgIn, x, y, kernel, width, height, kernelSize, k, distance_thr,cos_w,cos_h);
          }else{
